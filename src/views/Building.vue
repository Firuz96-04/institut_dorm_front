...
const buildingEdit = ref({})
const buildingDelete = ref({})
...
const deleteHandle = (data) => {
    ...
    buildingDelete.value = build
    openDelete.value = true
};

const editHandle = (data) => {
    const build = JSON.parse(JSON.stringify(data))
    buildingEdit.value = build
    openEdit.value = true
}
...
<template>
    <div class="card my_table">
        <DataTable v-model:selection="selectedProduct" :value="buildings" 
            :loading="loading"
            class="p-datatable-sm" showGridlines scrollable scrollHeight="400px">
            ...
            <Column field="floor_count" header="Этажность" headerClass="font-semibold column-text-center" class="text-center" style="width: 15%">
                <template #body="{ data }">
                    <span class="my_column">{{ data.floor_count }}</span>
                </template>
            </Column>
    ...
    <Teleport to="body">
        ...
        <deleteModal :visible="openDelete" :building="buildingDelete" @close="openDelete = false" /> 
        <editModal :visible="openEdit" :building="buildingEdit" @close="openEdit = false" />
    </Teleport>
</template>
